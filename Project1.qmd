...
```{r}
multi_year_api(year = c(2022, 1000, 1000), num="JWAP", cat = "SEX", loc_type= "DIVISION")

```

```{r}
# Functions to convert JWAP/JWDP to minutes, based on example provided by Dr. Post:
initial_convert_to_time = function(input_url){ # change given vector to time
    if # ADD CONDITIONAL HERE FOR JWAP VS JWDP
...
full_minute_conversion = function(input_col){
    data = initial_convert_to_time() 
    mins = calculate_minutes_to_midnight(data)
    colnames(mins) = "mins"
    mins = as.data.frame(mins)
    # The indexing on JWAP/JWDP counts up from 1; we can use the values to index our JWAP_mins to transform to the appropriate values:
    for (i in 1:length(input_col)){
      if (input_col[i] == 0){
        next
      } else {
        val = input_col[i]
        input_col[i] = mins$mins[val]
      }
    }
  return(input_col)
}
input_col = tibble_name$JWAP
input_col[4]

input_url = "https://api.census.gov/data/2022/acs/acs1/pums?get=JWAP,SEX,PWGTP&for=state:19"
tibble_maker(input_url)
JWAP_mins$JWAP_mins[179]
...
```{r}
# let's recreate our tibble_maker() function with added capabilities
input_url = "https://api.census.gov/data/2022/acs/acs1/pums?get=JWDP,SEX,PWGTP&for=state:19"
...
tibble_maker = function(input_url, tibble_name){
  ...
  tibble_name = as_tibble(parse, .name_repairs = 'unique') # specify rownames are in matrix
  tibble_name = tibble_name[-1,]# drop the row which has the tibble names in it
  tibble_name$JWAP = as.numeric(tibble_name$JWAP)
  # this is where we add the new components to adjust each var type!
  if ("JWAP" %in% names(tibble_name)){
    tibble_name$JWAP = full_minute_conversion(tibble_name$JWAP)
  }
  if ("JWDP" %in% names(tibble_name)){ # use same function for both time vars
    tibble_name$JWDP = full_minute_conversion(tibble_name$JWDP)
  }
}
tibble_maker(input_url)
```
...
JWAP_data = convert_to_time(tibble_name$JWAP) 
    JWAP_mins = calculate_minutes_to_midnight(JWAP_data)
    colnames(JWAP_mins) = "JWAP_mins"
    tibble_name$JWAP2 = tibble_name$JWAP
    # The indexing on JWAP/JWDP counts up from 1; we can use the values to index our JWAP_mins to transform to the appropriate values:
    for (i in 1:nrow(tibble_name)){
      if (tibble_name$JWAP[i] == 0){
        next
      } else {
        val = tibble_name$JWAP[i]
        tibble_name$JWAP[i] = JWAP_mins$JWAP_mins[val]
      }
    }
  }
```
