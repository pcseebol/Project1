...
```{r}
one_year_api = function(year = 2022, num = 'AGEP', cat = 'SEX', loc = "ALL"){
  ...
  returned_tibble[[3]] = as.numeric(returned_tibble[[3]])
   if (num == "JWAP"){
    ...
    separate_wider_delim function to isolate times from the time range string, and then a combination of dplyr's mutate, lubridate's parse_hm and a basic arithmetic calculation to find the midpoin
    
    returned_tibble$JWAP = returned_tibble$JWAP*2.5 # convert to minutes since midnight, aligned with midpoint of each interval. 0 is not effected.
   }
  if (num == "JWDP"){
    print('AHHHHHHHHHHHHHHHHH!!!!!!!') # need a way to standardize this
  }
return(returned_tibble)
}
test_api = one_year_api(year= 2010, num="JWAP", cat = "SEX", loc = "DIVISION")
test_api
```
...
convert_to_time = function(input_vec){
  JWAP <- temp_list$values$item
    ...
    # FIGURE OUT HOW TO GET VALUES OUT OF THIS DF NEXT
    lubridate::as.difftime(parsed_JWAP[,1], parsed_JWAP[,2])
    return(parsed_JWAP)
}
test = convert_to_time(temp_list)
```
